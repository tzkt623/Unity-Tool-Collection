...
{
    public class CSVTitleAttribute : Attribute
    {
        public int order;
        public Type converterType;

        private ICustomConverter converter;
        public ICustomConverter Convert
        {
            get
            {
                ...
                if (converter == null)
                    converter = (ICustomConverter)System.Reflection.Assembly.GetExecutingAssembly().CreateInstance(converterType.FullName);

                return converter;
            }
        }

        public CSVTitleAttribute(int _order)
        {
            order = _order;
            converterType = null;
        }

        public CSVTitleAttribute(int _order, Type _type)
        {
            order = _order;
            converterType = _type;
        }
