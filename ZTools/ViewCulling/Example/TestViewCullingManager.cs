...
{
    sealed class TestViewCullingManager : MonoBehaviour
    {
        void Start()
        {
            var group = ViewCullingManager.Instance.RequestNewGroup(0, 2048);

            group.SetCullingGroupCamera(Camera.main);
            group.SetWatchPoint(transform);
            group.SetDistanceBands(30, 50, 70);
        }

        void Update()
        {
            //ViewCullingManager.Instance.SetWatchPoint(transform);
        }
